// main.tf
// Root Terraform configuration for the AWS dev environment.
// This file wires together the reusable modules into a complete stack:
// - VPC networking
// - IAM baseline roles
// - ECR repository
// - ALB
// - ECS Fargate service
// - Postgres-lite EC2 instance
...
module "iam" {
  ...
  project          = var.project
  environment      = var.environment
  ci_principal_arn = "arn:aws:iam::${var.account_id}:role/github-actions" # TODO: replace with real CI principal ARN
}
...
// Security group for ALB, allowing inbound HTTP (and optionally HTTPS).
resource "aws_security_group" "alb_sg" {
  ...
  ingress {
    ...
    description = "Public HTTP access (demo)"
  }
